...
import { generateThesisStructure } from './StructureGenDip';
...
const StructureDiploma = ({ workType, language, speciality, workTopic, setStructure, visible, onClose, structure }) => {
    ...
    const StructureGen = async () => {
        setLoading(true); // Включаем индикатор загрузки

        try {
            if (!workTopic || workTopic.trim() === '') {
                // Если тема работы пустая, показываем тоаст с ошибкой
                Toast.show({
                    type: 'error',
                    position: 'bottom',
                    text1: 'Ошибка',
            ...
            // Pass workTopic, workType, speciality along with the existing parameters
            const newStructure = await generateThesisStructure(workTopic, language, workType, speciality, setStructure);

            if (newStructure.length > 0) {
                setStructure(newStructure);
            } else {
                // Custom Toast for when the topic is missing
                Toast.show({
                    type: 'error',
                    position: 'bottom',
                    text1: 'Ошибка',
        ...
        <Modal visible={visible} animationType="none" transparent>
            <View style={styles.modalContainer}>
                <Animated.View style={[styles.modalContent, { opacity: fadeAnim }]}>
                    ...
                    <Text style={styles.topicText}>Тема: {workTopic}</Text>
                    ...
                    >
                        {structure.map((item, index) => (
                            <View key={index} style={styles.sectionContainer}>
                                ...
                                {item.subSections && item.subSections.map((subItem, subIndex) => (
                    ...
                    <TouchableOpacity style={styles.generateButton} onPress={StructureGen}>
                        {loading ? (
...
export default StructureDiploma;
